...
export class PantallaVenderComponent implements OnInit {
  ...
  public vendedorRegistro: Vendedor;
  public imageVendedor: any;
  ...
  getInfoVendedorLoginById(id: number) {
    this.usuarioService.getInfoVendedorById(id).subscribe(
      ...
      () => {
        this.logueado = true;
        this.router.navigate(["p-vendedor"], {
          queryParams: {
  ...
  registroVendedor() {
    this.snackBar.open(
      "¡Gracias por registrarte! Tu solicitud de vendedor ha sido creada con éxito, en poco tiempo te enviaremos un correo con respuesta a tu solicitud. :)",
      "Cerrar"
    );
